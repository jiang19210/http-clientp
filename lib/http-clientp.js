...
exports.request = function (options, callback, httpcontext) {
    ...
    if (!options.agent) {
        if (httpcontext.request.proxy && 'https:' === protocol) {
            ...
            agent = this.getAgent(protocol, options.host, options.port);
...
exports.getAgent = function (protocol, host, port) {
    let key = protocol + ':' + host + ':' + port;
    let agent = httpAgents[key];
    if (agent == null && protocol === 'http') {
        agent = httpAgents[key] = new http.Agent({'keepAlive': true, maxFreeSockets: 20});
    } else {
        agent = httpAgents[key] = new https.Agent({'keepAlive': true, maxFreeSockets: 20});
    }
    return agent;
};